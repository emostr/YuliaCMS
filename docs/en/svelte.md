...
A block in [Liquid with htmx]({{ '/en/htmx/' | relative_url }}) covers nearly everything. Sometimes, though, you need logic **inside the browser** with no round trip to the server: a calculator, a configurator, an interactive diagram. That is what Svelte blocks are for.
